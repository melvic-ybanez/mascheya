...
instance Display SExpr where
  ...
  display (SAppExpr (SApp func args' _)) = display func ++ displayArgs args'
    where
      displayArgs (h :| t) =
        [Lexemes.space] ++ display h ++ case t of
          [] -> ""
  ...
  display (SLambdaExpr (SLambda params body)) =
    display $
...
instance Display SDefNel where
  display (SDefNel defs) =
    intercalate
      [Lexemes.semicolon, Lexemes.newline]
      $ display <$> toList defs

instance Display SPat where
  display (SVarPat var) = display var
  display (SLitPat lit _) = display lit
  display (SConstructorPat constructor args') =
    display constructor
      ++ [Lexemes.space]
      ++ (display $ SSV args')
