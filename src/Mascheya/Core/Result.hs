...
data Failure
  = ParseError ParseError Loc
  | RuntimeError RuntimeError Loc
...
instance Display Failure where
  ...
  display (TranslationError (UnableToTranslate error loc)) =
    formatMsgWithLoc "Translation Error" loc
      ++ error
